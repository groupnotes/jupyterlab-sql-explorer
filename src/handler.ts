...
export async function get<T>(act:string, params: {[key: string]: string}, options?: RequestInit): Promise<T> {
    let rc!: T
    try {
        rc=await requestAPI<any>(act + '?'+ (new URLSearchParams(params)).toString(), options)
    }catch(reason) {
        console.error(
...
export async function del<T>(act:string, params: {[key: string]: string}): Promise<T> {
    let rc!: T
    try {
        rc=await requestAPI<any>( act + '?'+ (new URLSearchParams(params)).toString(), { method: 'DELETE'})
    }catch(reason) {
        console.error(
            `The jupyterlab-sql-explorer server extension appears to be missing.\n${reason}`
        );
